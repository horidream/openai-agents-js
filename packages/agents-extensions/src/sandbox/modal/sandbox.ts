...
function adaptModalModule(modal: typeof import('modal')): ModalModule {
  return {
    // Keep the optional provider dependency behind our structural adapter.
    // Modal's nominal SDK types can vary independently from this boundary even
    // when the runtime surface consumed below remains compatible.
    ModalClient: modal.ModalClient as unknown as ModalModule['ModalClient'],
  };
}
