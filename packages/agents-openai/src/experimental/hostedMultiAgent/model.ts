...
export class OpenAIHostedMultiAgentModel extends OpenAIResponsesModel {
  ...
  protected _createResponsesWebSocket(
    ...
    // OpenAIClient intentionally accepts both the ESM and CommonJS declaration
    // graphs. ResponsesWS is tied to the declaration graph resolved here, so
    // bridge the nominally distinct client types at this runtime boundary.
    return new ResponsesWS(
      options.client as unknown as ConstructorParameters<typeof ResponsesWS>[0],
      {
